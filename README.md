...
- ページ切り替えは、ボタンを押した本人だけに見えるエフェメラル表示で行います。
...
- 通常は新着順の1ページ目だけを監視
- 1ページ目に既知商品が1件もなければ、前回取得分との境界を探すため2ページ目も確認
...
通知漏れした特定の1商品だけを手動送信する場合は、`Run workflow`で`mode`を`product-notify`にし、`product-id`へ12桁の商品IDを入力します。ページ内の連続した範囲をまとめて送る場合は、`page-number`へページ番号、`through-product-id`へ範囲の末尾となる商品IDを入力します。商品情報はフジヤエービックから取得され、通常通知として1通にまとめて送信されます。同じ商品構成は24時間以内に再実行しても二重投稿されません。
...
- 通常は公開ページ1枚だけを毎日20:00〜22:50（JST）の10分間隔で取得します。1ページ目に既知商品がない場合だけ2ページ目も取得し、全ページ巡回は行いません。
